...
def analyze_speaker_lengths(df):
    ...
    exceed_limit_count = sum(1 for count in combined_token_counts if count > 900)
...
print(f"Conversations Exceeding 900 Tokens: {results['token_stats']['combined']['exceed_limit_count']} ({results['token_stats']['combined']['exceed_limit_percentage']:.2f}%)")
...
tokenizer = AutoTokenizer.from_pretrained("answerdotai/ModernBERT-base")
def check_token_lengths(df, max_len=900):
...
class TutorDataset(Dataset):
    def __init__(self, df, max_len=900):
...
class MultiTaskModernBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Instead of out.pooler_output, use the last hidden state's CLS token
        x = out.last_hidden_state[:, 0, :]  # Get the [CLS] token representation
        x = self.dropout(x)
...
def train_loop(model, tr_loader, val_loader, device, epochs=100, lr=2e-5, patience=10):
    ...
    ts=datetime.now().strftime('%Y%m%d_%H%M%S'); outdir=f"output_modernBERT_CR_FL_{ts}"; os.makedirs(outdir,exist_ok=True)
    ...
    # test metrics
    test_loader=DataLoader(TutorDataset(test_df),batch_size=8)
...
if __name__=='__main__':
    ...
    tr_dl=DataLoader(TutorDataset(train_df),batch_size=8,shuffle=True)
    vl_dl=DataLoader(TutorDataset(val_df),batch_size=8)
